...
library("ggdendro")
library("grid")
setwd('F:/Geisenheim_PhD/Bea stuff/5_WINES FERM/')
...
beer <- readxl::read_xlsx('2020.12.14_Bea_Niel_13_MT_T29_T46_all_acids_Ethanol_Zucker_send.xlsx',sheet = 1,range = 'A4:P22', col_names = F) %>% t() %>% as.data.frame() %>% unite(x, c(V1,V2), sep = " ", remove = T,na.rm=T) %>% t() %>%as.data.frame() %>%janitor::row_to_names(1)%>%tibble::remove_rownames() 
...
## unique names
d <- c('I', 'II', 'III')
a <-  c("E1_18","EC1118_18","E1_10","EC1118_10")
b <- 'Most'
...
beer3$sample <- factor(beer3$sample, levels=c("E1_18","EC1118_18","E1_10","EC1118_10",'Most'))
##
...
p + geom_errorbar(aes(ymin=value-sd, ymax=value+sd), colour="black", width=.1, position=position_dodge(0.05)) +
  ...
  scale_fill_manual(
    name="Samples",values = c("E1_18"='red1',"E1_10"='red4',"EC1118_18"='blue1',"EC1118_10"='blue4', "Most"= 'grey'), # sample color scheme
