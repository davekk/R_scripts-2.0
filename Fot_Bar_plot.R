...
beer <- readxl::read_xlsx('210201_FOT deletion aromas.xlsx',sheet = 1,range = 'A4:AB45', col_names = T) 
...
beer[,-c(1,2)] <- apply(beer[,-c(1,2)],2,as.numeric) # convert the df from factor to numeric

 

beer2 <- beer[,colSums(beer!=0)>0]

names(beer2)[3:ncol(beer2)] <- 
  subset(sup.label,sup.label$old %in% names(beer2)[3:ncol(beer2)])[,2] %>% unlist() %>% as.character()
return(beer2)
}
...
beer2 %>% reshape2::melt(id=c('Sample','date'))%>% tidyr::separate(Sample, into=c('sample','rep'),sep="_") ->beer3 

beer4 <- summarySE(beer3,groupvars = c("sample",'date',"variable"), measurevar = 'value')
...
beer4$date <- as.factor(beer4$date)
##


p <-beer4 %>% filter(date=='01.02.21') %>% #filter(!is.na(sample)) ->b#%>%
...
p + geom_errorbar(aes(ymin=value-sd, ymax=value+sd), colour="black", width=.1,position=pd) +
  ...
  scale_fill_manual(
    name="Samples", values = c("E1 (S.uvarum)"='red',"EC (EC1118)"='blue',"F5 (GMO)"= 'green',"F11 (GMO)"='lightgreen',"F12 (GMO)"='darkgreen',"MT-Müller Th. C-must"='grey'),  guide=guide_legend(reverse = F)) +
