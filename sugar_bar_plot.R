...
beer <- readxl::read_xlsx('sugar and ethanol raw data for MT and chardnonnay.xlsx',sheet = 1,range = 'A4:P12', col_names = F) %>% t() %>% as.data.frame() %>% unite(x, c(V1,V2), sep = " ", remove = T,na.rm=T) %>% t() %>%as.data.frame() %>%janitor::row_to_names(1)%>%tibble::remove_rownames() 
...
beer3$sample <- factor(beer3$sample, levels=c("E1","EC1118"))
...
p + geom_errorbar(aes(ymin=value-sd, ymax=value+sd), colour="black", width=.1, position=position_dodge(0.05)) +
  ...
  scale_fill_manual(#c("EC118","ptr2-delta","ptr2-locus","E1",'Most')
    name="Samples",values = c("EC1118"='blue',"E1"='red'), # sample color scheme
...
ggsave('./sugar_ferm_bar_plots all free scale.png',width = 15,height = 15,units = 'in')
