...
beer <- readxl::read_xlsx('2_nitrogen boosted sugars.xlsx',sheet = 1,range = 'A4:P31', col_names = F) %>% t() %>% as.data.frame() %>% unite(x, c(V1,V2), sep = " ", remove = T,na.rm=T) %>% t() %>%as.data.frame() %>%janitor::row_to_names(1)%>%tibble::remove_rownames() 
# read the procossed ankom excel from teh first sheet
names(beer)[1] <- 'sample'
beer <- beer[-c(seq(from=4,to=27, by=4)),-c(14,15)]
#beer <- beer[complete.cases(beer),] 
## unique names
d <- c('I', 'II', 'III')
a <-  c("E1","EC1118","CBS7001")
b <- 'Most'
ba <- c('0','1') # 0-plus, 1-minus


#
beer$sample <- c(paste(rep(a,each =6),d,ba, sep = " "),paste(rep(b,each =2),d[1],ba, sep = " "))
...
beer %>% reshape2::melt(id='sample')%>% tidyr::separate(sample, into=c('sample','rep','condition'),sep=" ") ->beer2 

beer3 <- summarySE(beer2,groupvars = c("sample","variable",'condition'), measurevar = 'value')

# turn all the name column into factor
beer3$sample <- factor(beer3$sample, levels=c("E1","EC1118","CBS7001",'Most'))
##
sup.label <- c('Wine must','Wine must with\n Nitrogen supplementation')
names(sup.label) <- 0:1
p <- 
beer3 %>% dplyr::filter( condition==0) %>%  # 0-plus, 1-minus
...
#  ggtitle("Nitrogen fermentation") +
  ...
  scale_fill_manual(
    name="Samples",values = c("E1"='red',"EC1118"='blue',"CBS7001"='darkgreen', "Most"= 'grey'), # sample color scheme
...
#ggsave('./nboost/nboost_bar_plots  sugars free scale2.png')#,width = 15,height = 15,units = 'in')
