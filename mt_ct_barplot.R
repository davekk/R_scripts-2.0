...
beer <- readxl::read_xlsx('mt and chard.xlsx',sheet = 1,range = 'A4:P20', col_names = F) %>% t() %>% as.data.frame() %>% unite(x, c(V1,V2), sep = " ", remove = T,na.rm=T) %>% t() %>%as.data.frame() %>%janitor::row_to_names(1)%>%tibble::remove_rownames() 
# read the procossed ankom excel from teh first sheet
names(beer)[1] <- 'sample'

beer <- beer[-c(seq(from=0,to=15, by=4)),-c(14,15,16)]
...
#
beer$sample <-  c(paste(rep(a,each=3),d,sep=" ")) 
...
beer3$sample <- factor(beer3$sample, levels=c("E1_MT","EC1118_MT","E1_CH","EC1118_CH"))
##
...
p + geom_errorbar(aes(ymin=value-sd, ymax=value+sd), colour="black", width=.1, position=position_dodge(0.05)) +
  ...
  scale_fill_manual(#c("E1_MT","EC1118_MT","E1_CH","EC1118_CH")
    name="Samples",values = c("EC1118_CH"='blue4',"E1_CH"='red4',"EC1118_MT"='blue',"E1_MT"='red1'), # sample color scheme
...
ggsave('./MT_ct_ferm_bar_plots ethanol free scale.png',width = 15,height = 15,units = 'in')
